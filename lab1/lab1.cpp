...
void test_ModelARX_skokJednostkowy_1()
{
	...
	try
	{
        ...
        instancjaTestowa.zapiszDoPliku("testZpliku_1.txt");
...
void test_ModelARX_skokJednostkowy_2()
{
	...
	try
	{
        ...
        //Serializacja do pliku (JSON)
        instancjaTestowa.zapiszDoPliku("testZpliku_2.txt");
...
void test_ModelARX_skokJednostkowy_3()
{
	...
	try
	{
        ...
        //Serializacja do pliku (JSON)
        instancjaTestowa.zapiszDoPliku("testZpliku_3.txt");
	}
	catch (...)
	{
		std::cerr << "INTERUPTED! (niespodziwany wyjatek)\n";
...
//Test z pliku
//----------------------------------
void test_ModelARX_z_pliku()
{
	//Sygnatura testu:
	std::cerr << "\n ModelARX z pliku -> test skoku jednostkowego nr 3: \n";
	try
	{
		// Przygotowanie danych:
		ModelARX instancjaTestowa({ -0.4,0.2 }, { 0.6, 0.3 }, 2, 0);
        ...
        //Serializacja do pliku (JSON)
        instancjaTestowa.zapiszDoPliku("testZpliku.txt");

        //=============Odczyt================

        ModelARX sprawdzenie("testZpliku.txt");
        constexpr size_t LICZ_ITER2 = 30;
		std::vector<double> sygWe2(LICZ_ITER2);      // pobudzenie modelu, 
		std::vector<double> spodzSygWy2(LICZ_ITER2); // spodziewana sekwencja wy
		std::vector<double> faktSygWy2(LICZ_ITER2);  // faktyczna sekwencja wy
		// Symulacja modelu:
		for (int i = 0; i < LICZ_ITER2; i++)
			faktSygWy2[i] = sprawdzenie.symuluj(sygWe2[i]);

		// Weryfikacja poprawności i raport:
		if (porownanieSekwencji(spodzSygWy2, faktSygWy2))
			std::cerr << "OK!\n";
		else
		{
			std::cerr << "FAIL!\n";
			raportBleduSekwencji(spodzSygWy2, faktSygWy2);
		}
	}
	catch (...)
	{
